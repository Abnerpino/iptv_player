...
=> Fields: The 'clients' fields are specified in the 'handleRegisterDevice' function of 'screen_activation.jsx' (the 'force_update' field triggers a 'silent notification' that forces the update of client information). The 'notifications' fields are three: 'clients_id' (to store the IDs of the clients to whom you want to show the notification), of type Array of Strings, 'message' (to store the notification message), of type String and 'type' ('initial' for initial notifications and 'other' for all other notifications) of type String. The 'resellers' fields are six: 'bank', 'country_code', 'email', 'name', 'number_card' and 'whatsapp', all of type String. The 'keys' fields are two: 'key' and 'type' ('TMDB' for example), both of type String.
